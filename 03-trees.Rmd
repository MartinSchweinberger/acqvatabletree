...
```{r acqvalab3, echo=F, fig.cap="", message=FALSE, warning=FALSE, out.width='100%'}
...
The data now consists of factors with two levels each. 
...
```{r cit24, message=FALSE, warning=FALSE}
...
The argument `iter` refers to the number of iterations to run. According to @breiman2001random[pp. 4 - 6]  iterations is usually good enough. With this data set (if it had NAs) and when we were to execute the code, the resulting OOB-error rates lie somewhere around 17 and 18 percent. When we were to set `iter` to 20, we get values a little better and a little worse, so doing more iterations doesn't improve the situation.
